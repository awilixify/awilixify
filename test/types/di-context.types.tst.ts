...
describe("DIContext providerOverrides", () => {
	class LocalService {
		private declare readonly __brand: never;
	...
	it("rejects imported providers in root providerOverrides", () => {
		expect(
			DIContext.create(RootModule, {
				providerOverrides: {
...
describe("DIContext moduleOverrides", () => {
	...
	it("rejects unknown module override provider and pre-handler keys", () => {
		expect(
			overrideModule(RootModule, {
				providers: {
					missingService: new LocalService(),
				},
			}),
		).type.toRaiseError();
